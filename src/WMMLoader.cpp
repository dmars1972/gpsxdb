...
bool WMMLoader::load(double year,
    ...
    // The wmm raster is a static snapshot for whatever year it was computed
    // at (see the `year` param to load()), not a live model -- so there's
    // no p_year param to accept here, only p_srid (mirrors
    // elevation_at_point_ft's pattern in TerrainLoader.cpp). Unlike
