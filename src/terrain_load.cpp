// Standalone terrain elevation loader (USGS 3DEP or Copernicus DEM GLO-30).
// Usage: terrain_load -s <host> -d <db> -u <user>
//                      (--bbox minlon,minlat,maxlon,maxlat | --global) [-4]
//                      [--source 3dep|copernicus]
//                      [--threads <n>]
// Requires ~/.pgpass for authentication (no -p/password flag).
#include "TerrainLoader.h"
#include <iostream>
#include <string>
#include <sstream>
#include <algorithm>
#include <unistd.h>

int main(int argc, char** argv) {
    std::string server, database, user;
    double min_lon = 0, min_lat = 0, max_lon = 0, max_lat = 0;
    bool have_bbox = false;
    bool global = false;
    TerrainSource source = TerrainSource::USGS3DEP;
    int dest_srid = 3857;
    int threads = 4;

    for (int i = 1; i < argc; ++i) {
        std::string arg = argv[i];
        if      ((arg == "-s") && i+1 < argc) server   = argv[++i];
        else if ((arg == "-d") && i+1 < argc) database = argv[++i];
        else if ((arg == "-u") && i+1 < argc) user     = argv[++i];
        else if  (arg == "-4")                dest_srid = 4326;
        else if  (arg == "--global")          global = true;
        else if ((arg == "--threads") && i+1 < argc) threads = std::stoi(argv[++i]);
        else if ((arg == "--source") && i+1 < argc) {
            std::string v = argv[++i];
            if (v == "3dep") source = TerrainSource::USGS3DEP;
            else if (v == "copernicus") source = TerrainSource::CopernicusGLO30;
            else { std::cerr << "Error: --source must be '3dep' or 'copernicus'\n"; std::cerr.flush(); _exit(1); }
        }
        else if ((arg == "--bbox") && i+1 < argc) {
            std::string v = argv[++i];
            std::replace(v.begin(), v.end(), ',', ' ');
            std::istringstream iss(v);
            if (!(iss >> min_lon >> min_lat >> max_lon >> max_lat)) {
                std::cerr << "Error: --bbox must be minlon,minlat,maxlon,maxlat\n";
                std::cerr.flush();
                _exit(1);
            }
            have_bbox = true;
        }
        else if (arg == "-h" || arg == "--help") {
            std::cout << "Usage: terrain_load -s <host> -d <db> -u <user>\n"
                         "                     (--bbox minlon,minlat,maxlon,maxlat | --global)\n"
                         "                     [-4 (WGS84 instead of Mercator)]\n"
                         "                     [--source 3dep|copernicus, default 3dep (US-only, higher accuracy);\n"
                         "                       copernicus is near-global but overlaps 3dep in the US if used there]\n"
                         "                     [--threads <n>, default 4]\n"
                         "\n"
                         "  --global loads Copernicus DEM GLO-30 for every populated non-US region\n"
                         "  on the planet in one call (19 regions covering everywhere except the\n"
                         "  continental US -- 3DEP is authoritative there, load separately with\n"
                         "  --source 3dep -- and Antarctica). Ignores --bbox and --source.\n"
                         "\n"
                         "Requires ~/.pgpass for authentication.\n";
            std::cout.flush();
            _exit(0);
        }
    }

    // _exit (not return) on every exit path, including these validation
    // errors — a normal return from main() runs static destructors that
    // double-free across PROJ/pqxx (see the success path below).
    if (server.empty() || database.empty() || user.empty()) {
        std::cerr << "Error: -s, -d, -u are required\n";
        std::cerr.flush();
        _exit(1);
    }
    if (!global && !have_bbox) {
        std::cerr << "Error: --bbox or --global is required\n";
        std::cerr.flush();
        _exit(1);
    }
    if (global && have_bbox) {
        std::cerr << "Error: --bbox and --global are mutually exclusive\n";
        std::cerr.flush();
        _exit(1);
    }

    TerrainLoader loader(server, user, database);
    bool ok = global
        ? loader.loadGlobal(dest_srid, threads, true)
        : loader.load(min_lon, min_lat, max_lon, max_lat,
                      source, dest_srid, threads, true);
    std::cout.flush();
    // _exit skips static destructor teardown that causes double-free with PROJ/pqxx
    _exit(ok ? 0 : 1);
}
