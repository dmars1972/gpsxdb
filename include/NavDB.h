...
public:
    ...
    // Idempotent counterpart to initializeSchema() for the regional
    // installer: CREATE TABLE/INDEX IF NOT EXISTS for every table with
    // fixed DDL, no DROPs — safe to run against an empty target database
    // or one that already has some regions installed. Does NOT create
    // public.terrain: that table's DDL is generated by raster2pgsql from
    // actual tile metadata (pixel type, scale, SRID-specific constraints),
    // not fixed like the tables here, so regional_install applies a DDL
    // snippet captured from the source database instead (see
    // regional_db_export's terrain.schema.sql output).
    void ensureSchema();
